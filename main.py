...
HORSE_MIN_PLAYERS = 3             # 최소 참가 인원 (미만이면 무효)
HORSE_MAX_PLAYERS = 6             # 최대 참가 인원
HORSE_2ND_REWARD = 300            # 2등 보상 ($WHAT)
...
async def _run_horse_race(
    ...
    # 2등 보상: 300 WHAT
    second_horse = result_order[1] if len(result_order) >= 2 else None
    second_uid = horse_to_uid.get(second_horse) if second_horse else None
    if second_uid is not None:
        async with get_user_lock(chat_id, second_uid):
            uref2 = user_ref(db, chat_id, second_uid)
            snap2 = uref2.get()
            udata2 = snap2.to_dict() if snap2.exists else {}
            new_total = int(udata2.get("total_exp", 0)) + HORSE_2ND_REWARD
            uref2.set(
                {
                    "total_exp": new_total,
                    "last_seen": dt,
        ...
        second_display = horse_to_display.get(second_horse, str(second_uid))
        try:
            await bot.send_message(
                chat_id=chat_id,
                text=f"🥈 {second_display}님의 {second_horse}이(가) 2등! {HORSE_2ND_REWARD}$WHAT가 지급되었습니다!",
            )
        except Exception:
            pass
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text.strip() == "!얼룩이덜룩이":
        ...
        status_text = (
            ...
            f"최소 {HORSE_MIN_PLAYERS}명 ~ 최대 {HORSE_MAX_PLAYERS}명 | 1분 내 말을 선택하세요!\n\n"
            f"🥇 1등: 강화 방어권 1장\n"
            f"🥈 2등: {HORSE_2ND_REWARD}$WHAT\n\n"
            f"말을 선택하면 자동으로 참가 (참가비 {HORSE_BET_COST}$WHAT)\n"
            f"개설자도 말을 선택해야 참가됩니다!\n"
            f"{HORSE_MIN_PLAYERS}명 미만 시 자동 취소 및 환불\n\n"
    ...
    if text.strip() == "!가이드":
        await update.message.reply_text(
            ...
            f"  최소 {HORSE_MIN_PLAYERS}명~최대 {HORSE_MAX_PLAYERS}명, 1분 제한\n"
            f"  🥇1등: 강화방어권 | 🥈2등: {HORSE_2ND_REWARD}$WHAT\n"
            f"  {HORSE_MIN_PLAYERS}명 미만 시 자동 취소/환불\n"
            "\n"
            "[띱 이벤트]\n"
            ...
            if has_fish_sell:
                for name, cnt in sorted(fish_inv.items()):
                    ...
                    rows.append(
                        [
                            InlineKeyboardButton(
                                text=f"{name} x{int(cnt)} 판매 (+{sub}$WHAT)",
                                callback_data=f"fs1:{chat_id}:{user_id}:{name}",
                            )
                        ]
                    )
                if note_cnt > 0:
                    rows.append(
                        [
                            InlineKeyboardButton(
                                text=f"{FISHING_SATOSHI_NOTE} x{int(note_cnt)} 판매 (+{note_total}$WHAT)",
                                callback_data=f"fs1:{chat_id}:{user_id}:{FISHING_SATOSHI_NOTE}",
                            )
                        ]
                    )
                rows.append(
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("fs1:"):
        parts = data.split(":", 3)
        if len(parts) != 4:
            return
        _, cid, uid, sell_name = parts
        if int(cid) != chat_id:
            return
        if q.from_user is None or int(q.from_user.id) != int(uid):
            try:
        ...
        today = kst_date_str(dt)
        target_user_id = int(uid)
        is_note = sell_name == FISHING_SATOSHI_NOTE
        async with get_user_lock(chat_id, target_user_id):
            uref = user_ref(db, chat_id, target_user_id)
            snap = uref.get()
            udata = snap.to_dict() if snap.exists else {}
            ...
            if is_note:
                ...
                if note_cnt <= 0:
                    try:
                        await q.message.edit_text(f"판매할 {sell_name}이(가) 없습니다.")
                    except Exception:
                        pass
                    return
                total = 100_000 * int(note_cnt)
                ...
                fish_inv = _coerce_int_dict(udata.get("fish_inventory"))
                cnt = int(fish_inv.get(sell_name, 0))
                if cnt <= 0:
                    try:
                        await q.message.edit_text(f"판매할 {sell_name}이(가) 없습니다.")
                    except Exception:
        ...
        try:
            await q.message.edit_text(f"{sell_name} 판매 완료! +{int(total)}$WHAT")
        except Exception:
            pass
        return
